...
public class ProductEventConsumer {
    ...
    private void pollLoop() {
        ...
        consumer.subscribe(List.of("product.plan.assigned", "product.did.purchased"));
        ...
        while (true) {
            for (ConsumerRecord<String, Map<String, Object>> record :
                ...
                if (record.topic().equals("product.plan.assigned")) {
                    tenantService.onPlanAssigned(
                            tenantId,
                            UUID.fromString(payload.get("planId").toString()),
                            payload.get("planCode").toString()
                    );
                }

                if (record.topic().equals("product.did.purchased")) {
                    tenantService.onDidPurchased(tenantId);
                }
            }
        }
    }
