...
public class ProvisioningOrchestratorImpl implements ProvisioningOrchestrator {
    ...
    private void executeStep(Tenant tenant, ProvisioningTask task, ProvisioningStep step) {
        ...
        Map<String, Object> result = switch (step) {
            ...
            case DEPLOY_KAMAILIO, DEPLOY_RTPENGINE, DEPLOY_COTURN, DEPLOY_WEBRTC_GW,
                 DEPLOY_ASTERISK -> executeDeployTelecomStep(tenant, step);
            case DEPLOY_AI_SERVICE, DEPLOY_AGENT_SERVICE, DEPLOY_CALL_CONTROL -> executeDeployServicesStep(tenant, step);
            case CREATE_SIP_LOADBALANCER, CREATE_WEBRTC_INGRESS -> executeCreateLoadBalancerStep(tenant, step);
    ...
    private Map<String, Object> executeCreateLoadBalancerStep(Tenant tenant, ProvisioningStep step) {
        return Map.of("step", step.name());
    }
    ...
    private boolean shouldSkipStep(Tenant tenant, ProvisioningStep step) {
        if (tenant.getDeploymentModel() == DeploymentModel.SHARED) {
            return switch (step) {
                case DEPLOY_INFRA_KAFKA, DEPLOY_INFRA_REDIS, DEPLOY_INFRA_POSTGRES, DEPLOY_INFRA_MYSQL -> true;
                default -> false;
            };
        }
        return false;
    ...
    private TenantStatus mapStepToSubstatus(ProvisioningStep step) {
        return switch (step) {
            ...
            case DEPLOY_ASTERISK -> TenantStatus.PROVISIONING_MEDIA_SERVER;
