...
public class LightingPlanController extends BaseController {
    ...
    @PatchMapping("/v1/shots/{shotId}/lighting-plan")
    public ResponseEntity<LightingPlanView> saveEdit(@PathVariable UUID shotId, @RequestBody SaveLightingPlanEditRequest request) {
        return ResponseEntity.ok(lightingPlanService.saveEdit(tenant().tenantId(), shotId, request));
    }
}
