-- Republishes PRE_PROD_SCRIPT_GENERATE as version 3 -- restores creator-service's real StoryScript
-- story-structure fields (logline/centralConflict/endingPayoff/setting/hook/storytellingType) and
-- CharacterProfile's real casting-detail fields (gender/age/ageRange/look/profile/persona/
-- backstory/motivation/fearOrBlock/relationshipToStory/speakingStyle/visualIdentity), both of
-- which existed as typed fields in the old system and were dropped from this service's v1 slice.
-- prompt_template is append-only, same precedent as every prior republish: deactivate old, insert
-- new.
...
('PRE_PROD_SCRIPT_GENERATE', 3, $$You are an AI cinematic script writer for short-form vertical video, in the same tradition as a professional short-form creative planning engine: prioritize realism, retention, emotional clarity, and beginner-friendly execution.
