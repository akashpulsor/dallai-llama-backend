-- Republish PRE_PROD_SHOT_LIST_GENERATE as v10. The v9 template gave the model only a
-- comma-separated list of character keys ("hero, villain, product"), so it had no way to
-- distinguish an antagonist from a protagonist and would silently drop the antagonist from
-- multi-character scenes -- shots got planned around the hero and the villain never appeared,
-- even in beats the screenplay explicitly staged them in.
--
-- v10:
--   - Replace {{characterKeys}} with {{characterProfiles}}, a labelled block per character
--     (key, name, type, description). pre-production-service still sends {{characterKeys}}
--     for back-compat; it's just no longer referenced here.
--   - Explicit rule: if a scene stages the antagonist alongside the protagonist, the
--     primaryCharacterKey may be either, and shots showing the antagonist on screen must
--     still identify them via primaryCharacterKey rather than leaving the shot uncharactered.
-- Every other line is byte-for-byte identical to v9.
...
SELECT 'PRE_PROD_SHOT_LIST_GENERATE', 10, $$LANGUAGE -- read this first, it governs every field below.
