-- Republishes PRE_PROD_SHOT_LIST_GENERATE as version 5 -- editingNotes was a single generic line
-- ("cut timing, speed ramp, or transition-in-edit notes") that the model answered per shot in
-- isolation, producing thin, interchangeable notes that didn't actually reference the shots
-- before/after. Rewrites it to explicitly require the cut INTO this shot, continuity carried
-- across that cut, and pacing rhythm relative to neighbors -- reasoning across the sequence the
-- same way a real editor's cut list does, not per-shot. Also sharpens screenDirection to call out
-- when it deliberately breaks continuity (should then be explained in editingNotes) versus just
-- describing itself in isolation. Everything else unchanged from v4.
