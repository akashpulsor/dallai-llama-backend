-- Republishes PRE_PROD_SHOT_LIST_GENERATE as version 7 to stop captureFormat carrying a
-- resolution.
--
-- Version 6 offered "e.g. 4K 10-bit log, standard 1080p" as the example, and the model copied it
-- literally: every shot in a real project came back with cine_capture_format = "standard 1080p".
-- The shot plan does not decide resolution -- the creator's project config or per-request override
-- does, and that argument is what the render actually uses. So a project rendering at 480p had
-- every one of its prompts asserting 1080p, a contradiction the model was left to resolve.
--
-- Fixed here rather than by stripping the value downstream: a consumer scrubbing a field it was
-- handed is a workaround that leaves the wrong data in the table for every other reader, and only
-- holds for as long as someone remembers the scrub exists. captureFormat describes the LOOK --
-- codec, bit depth, colour profile -- and the prompt now says so, and says explicitly not to put a
-- resolution in it.
