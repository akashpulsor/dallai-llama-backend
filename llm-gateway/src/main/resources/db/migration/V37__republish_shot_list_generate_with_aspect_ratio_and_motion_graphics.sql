-- Republishes PRE_PROD_SHOT_LIST_GENERATE as version 3 -- adds a project-level aspect ratio
-- directive ({{aspectRatio}}), motion-graphics classification guidance ({{motionGraphicsGuidance}}),
-- and MOTION_GRAPHIC as a valid shotType. Everything else (cinematography block, all other
-- fields) is unchanged from v2. prompt_template is append-only, same v1->v2 precedent (V23):
