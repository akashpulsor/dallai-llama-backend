-- Republishes PRE_PROD_SHOT_LIST_GENERATE as version 8 to stop descriptions being written in the
-- project's dialogue language.
--
-- The template is handed {{scriptText}}, which is written in whatever language the project speaks.
-- Nothing told the model that only some fields should follow it, so it mirrored the script's
-- language into everything -- a real project came back with a Hindi scriptLine describing on-screen
-- text ("स्क्रीन पर 'पर्सनलाइज़्ड एस्ट्रो-रिपोर्ट्स' ... दिखाई देते हैं") on a shot whose textOverlay the
-- creator had already set to English. That Hindi description then flows into the video-generation
-- prompt, where nothing can act on it.
--
-- Version 8 splits the two cases by what the field IS: descriptions are instructions to a director
-- and a video model, always English; voiceOver and textOverlay are the finished product an audience
-- sees and hears, in the project's language. {{dialogueLanguage}} is now passed by
-- ShotListGenerationService.
--
-- This is a generation-side fix, so it governs shots created from here on. Shots already stored
-- with Hindi descriptions keep them until their shot list is regenerated -- a migration cannot
-- translate prose, and guessing at a translation of a creator's scene description would be worse
-- than leaving it.
...
('PRE_PROD_SHOT_LIST_GENERATE', 8, $$LANGUAGE -- read this first, it governs every field below.

Write EVERY descriptive field in English: scriptLine, action, cameraNote, composition, expression, bodyLanguage, emotion, editingNotes, creatorDirection, directorNote, cinematicExecution, screenDirection, location, soundDesign, retentionGoal, sketchPrompt, rookieFriendlyGuide, and every field inside the cinematography object. English regardless of what language the script below is written in.
