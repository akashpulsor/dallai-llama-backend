-- Republishes PRE_PROD_SCRIPT_GENERATE as version 5 -- scriptText previously had no guidance at
-- all beyond "Write a short narrative script for this brief", which the model could (and did)
-- satisfy with anything from one sentence to several paragraphs. This makes the expectation
-- explicit: a comprehensive, complete telling of the story's full arc in prose -- not truncated
-- to an arbitrary word count, and not a shot-by-shot breakdown (that happens later, in shot list
-- generation) or a single-sentence logline (that is its own separate field below).
...
('PRE_PROD_SCRIPT_GENERATE', 5, $$You are an AI cinematic script writer for short-form vertical video, in the same tradition as a professional short-form creative planning engine: prioritize realism, retention, emotional clarity, and beginner-friendly execution.
