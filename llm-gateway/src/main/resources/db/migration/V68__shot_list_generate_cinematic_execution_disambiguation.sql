-- Confirmed live: an entire shot-list generation (11/11 shots) came back with cinematicExecution
-- null and the whole nested cinematography object null on every single shot -- not the prompt's
-- intended "leave cinematography fields null when not meaningful" judgment call, a systematic
-- failure across the whole response. The one occurrence already fixed with LenientStringDeserializer
-- (a duplicate cinematography object landing in one shot's cinematicExecution string field) turns
-- out to have been the tip of this: the model confuses "cinematicExecution" (a short prose string)
-- with the separate "cinematography" object below it, badly enough that for this whole response it
-- seems to have spent that content on the wrong field for every shot, leaving both empty. Fixes the
-- prompt itself now instead of just tolerating the bad shape: cinematicExecution is redescribed as
-- explicitly NOT a JSON object with a concrete example of what it should look like, and a one-line
-- reminder is added directly above the cinematography object clarifying it's a different field.
